...
//  $Id: ccedar_core.h 2060 2024-03-05 03:11:56Z ynaga $
...
namespace ccedar {
  ...
  enum error_code { NO_VALUE = -1, NO_PATH = -2 };
  // dynamic double array
  template <typename key_type, typename value_type, const int MAX_KEY_BITS = sizeof (key_type) * 8>
  class da {
  public:
    typedef typename to_unsigned <key_type>::type ukey_type;
    typedef value_type result_type;
    enum { MAX_KEY_CODE = 1 << MAX_KEY_BITS, MAX_ALLOC_SIZE = MAX_KEY_CODE << 4, MAX_TRIAL = 1 };
    struct node {
    ...
    value_type traverse (const key_type* key, size_t& from, size_t& pos, size_t len) const {
      union { int i; value_type x; } b = { _find (key, from, pos, len) };
      return b.x;
    }
    value_type& update (const key_type* key, size_t len) {
      if (! len) _err (__FILE__, __LINE__, "failed to insert zero-length key\n");
      size_t from (0), pos (0);
    ...
    const node* array () const { return _array; }
    void clear () {
  ...
  private:
    ...
    { std::fprintf (stderr, "%s [%d]: %s", fn, ln, msg); std::exit (1); }
    template <typename T>
    ...
    int _find (const key_type* key, size_t& from, size_t& pos, const size_t len) const {
      ...
      if (n.check != static_cast <int> (from)) return NO_VALUE;
      return n.base;
    }
